...
(function($){
	$.fn.slide=function(options){
	   ...
	   ool.find("b").mouseover(function(){	
		...
		$(this).find("li a").live("swipeleft",function(){
			if(num<3){
				num=ool.find("b").index($(".cur"))+1;
			}else{
				num=3;
			}	
			run ();
		})
		$(this).find("li a").live("swiperight",function(){
			if(num>0){
				num=ool.find("b").index($(".cur"))-1;
			}else{
				num=0;
		...
		function run(){
			ool.find("b").eq(num).addClass("cur").siblings().removeClass("cur");
				switch(opts.affect){
					...
					case 4:
						$box.find("li").css({" ":" "});
						$box.find("li").stop(false,true).hide(opts.speed).eq(num).show(opts.speed);
						break;	
				}
		}
