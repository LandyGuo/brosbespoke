...
import json
import xmltodict
from ShiXiongDeYiGui.settings import TOKEN,APPID, APP_SECRET, TOKEN_CACHE_PRE, TICKET_CACHE_PRE
import sys 
reload(sys) 
...
#
def get_token():
    token = cache.get(TOKEN_CACHE_PRE)
    if token:
        return token
    else:
        ...
        if token and expires_in:
            cache.set(TOKEN_CACHE_PRE, token, expires_in-60)
        return token

#获取jsapi_ticket
def get_ticket():
    ticket = cache.get(TICKET_CACHE_PRE)
    if ticket:
        return ticket
    else:
        ...
        expires_in = dict_data.get('expires_in')
        if ticket and expires_in:
            cache.set(TICKET_CACHE_PRE, ticket, expires_in-60)
        return ticket
