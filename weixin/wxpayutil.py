...
import os
from ShiXiongDeYiGui.settings import BASE_DIR
import logging
...
class WxPayConf_pub(object):
    ...
    APPSECRET = "d01620c3f1ef9b5d6885883d6942e10f"
    #受理商ID，身份标识
    MCHID = "1234875402"
    #商户支付密钥Key。审核通过后，在微信发送的邮件中查看
    #KEY = "kqlksjdo12op10ozSjaQOL91KSDI8USQ"
    KEY = "brosBespoke888888888888888888888"
    MY_BASE_URL = 'http://weixin.brosbespoke.com'
    #=======【异步通知url设置】===================================
    #异步通知url，商户根据实际开发过程设定
    NOTIFY_URL = MY_BASE_URL+"/weixin/wxpay_notify_url"

    #=======【JSAPI路径设置】===================================
    #获取access_token过程中的跳转uri，通过跳转将code传入jsapi支付页面
    JS_API_CALL_URL = MY_BASE_URL+"/weixin/wxpay_pay/pay?showwxpaytitle=1"

    #=======【证书路径设置】=====================================
    #证书路径,注意应该填写绝对路径
    SSLCERT_PATH = os.path.join(BASE_DIR, "/weixin/wxpay/cert/apiclient_cert.pem")
    SSLKEY_PATH = os.path.join(BASE_DIR, "/weixin/wxpay/cert/apiclient_key.pem")
...
class JsApi_pub(Common_util_pub):
    ...
    def getOpenId(self):
        ...
        get_logger().debug('------------------------getOpenId  openid='+self.openid)
        return self.openid
...
class UnifiedOrder_pub(Wxpay_client_pub):
    ...
    def getPrepayId(self):
        ...
        for key, value in self.result.items():
            debug_str+= "\"%s\":\"%s\"" % (key, value)
            
        get_logger().debug('------------------------getPrepayId  self.result='+debug_str)
